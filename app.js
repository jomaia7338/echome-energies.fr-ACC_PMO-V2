...
function setupMap(){
  ...
  // Clic : ALT = producteur ; sinon, si activé, ajoute un consommateur
  app.map.on('click', (e)=>{
...
function showWorstOverlay(w,ok){
  ...
  const mid={lat:(w.a.lat+w.a.lon? (w.a.lat+w.b.lat)/2 : w.a.lat), lon:(w.a.lon+w.b.lon)/2};
  // correct mid:
  mid.lat = (w.a.lat + w.b.lat)/2; mid.lon = (w.a.lon + w.b.lon)/2;
...
async function overpass(query){
  try{
    ...
    return await fetchJSON(OVERPASS_URL,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},body,timeoutMs:20000,retries:2,expect:'json'});
...
function labelFromTags(tags){
  ...
  const lut={school:'École',college:'Collège',university:'Université',hospital:'Hôpital',clinic:'Clinique',doctors:'Médecins',pharmacy:'Pharmacie',sports_centre:'Centre sportif',stadium:'Stade',pitch:'Terrain',swimming_pool:'Piscine',marketplace:'Marché',supermarket:'Supermarché',parking:'Parking',industrial:'Industriel'};
...
function buildProspectPopup(c){
  ...
  return `
    <div class="pp">
      ...
      <div class="muted small">↔ Prod: <b>${dprod!==null?dprod.toFixed(2)+' km':'—'}</b></div>
...
function overpassQueryAroundProducer(Dkm,cats){
  ...
  const r=Math.max(1,Math.round(Dkm*1000)); const {lat,lon}=app.producteur; const b=[];
...
async function scanProspects(){
  ...
  if(data && Array.isArray(data.elements)){
    ...
    norm.forEach(c=>{
      ...
      const mk = L.circleMarker([cObj.lat,cObj.lon], { radius:6, color, weight:2, fillOpacity:.7 }).addTo(app.layers.cand);
...
document.addEventListener('click', (e)=>{
  ...
  if(act==='cand-zoom'){ app.map.setView([c.lat, c.lon], Math.max(app.map.getZoom(), 17)); }
  if(act==='cand-sim'){
    const pts=[]; if(app.producteur) pts.push(app.producteur); app.participants.forEach(p=>pts.push(p)); pts.push(c);
    const w = worstPair(pts); const ok = (w.d <= app.distMaxKm); showWorstOverlay(w, ok);
  }
...
function fillProspectPanel(c){
  ...
  $('pp-add').onclick = ()=>{ addParticipant({ id:crypto.randomUUID(), nom:c.nom||'Prospect', lat:c.lat, lon:c.lon, type:'consumer' }); closeProspectPanel(); };
  $('pp-sim').onclick = ()=>{ const pts=[]; if(app.producteur) pts.push(app.producteur); app.participants.forEach(p=>pts.push(p)); pts.push(c); const w=worstPair(pts); const ok=(w.d<=app.distMaxKm); showWorstOverlay(w, ok); };
  $('pp-route').onclick = ()=>{ const url=`https://www.google.com/maps/dir/?api=1&destination=${c.lat},${c.lon}`; window.open(url,'_blank'); };
}
...
// ================== UI (onglets, barre mobile, bottom-sheet) ==================
function wireTabs(){
...
function wireUI(){
  ...
  if (fab && side) {
    ...
    document.addEventListener('click', (e)=>{
      ...
      const inside = side.contains(e.target) || fab.contains(e.target);
      if (!inside) { side.classList.remove('open'); document.body.classList.remove('no-scroll'); }
    });
  }
  // Drag poignée bottom sheet
  (function wireSheetDrag(){
    const grab = document.querySelector('.sheet-grabber');
    if(!side || !grab) return;
    let startY=0, startOpen=false, dragging=false;
    const onStart = (y)=>{ dragging=true; startY=y; startOpen=side.classList.contains('open'); };
    const onMove  = (y)=>{ if(!dragging) return; const dy=Math.min(1,Math.max(0,(y-startY)/240)); side.style.transform = startOpen?`translateY(${dy*62}vh)`: `translateY(${(1-dy)*62}vh)`; };
    const onEnd   = (y)=>{ if(!dragging) return; dragging=false; side.style.transform=''; const dy=Math.min(1,Math.max(0,(y-startY)/240)); const open = startOpen ? (dy<0.35) : (dy>0.35); side.classList.toggle('open', open); };
  ...
  // Mobile toolbar
  $('mtOptions')?.addEventListener('click', ()=> side?.classList.toggle('open'));
  $('mtLocate')?.addEventListener('click', ()=> $('btnLocate')?.click());
  $('mtScan')?.addEventListener('click', ()=> $('btnScan')?.click());
  document.querySelectorAll('.mt-chipset .chip')?.forEach(btn=>{
    btn.addEventListener('click', ()=>{
      document.querySelectorAll('.mt-chipset .chip').forEach(b=>b.classList.remove('active'));
  ...
  $('btnGeocode')?.addEventListener('click', async ()=>{
    ...
    setProducteur({ lat:r.lat, lon:r.lon }); if ($('addrFull')) $('addrFull').value = r.label || q;
    app.map.setView([r.lat, r.lon], 14); setStatus('Adresse localisée');
  });
  $('btnSetProducerFromInputs')?.addEventListener('click', async ()=>{
    ...
    setProducteur({ lat:la, lon:lo }, { doReverse:true }); app.map.setView([la, lo], 14);
  });
  $('btnLocate')?.addEventListener('click', ()=>{
    ...
    navigator.geolocation.getCurrentPosition(pos=>{
      ...
      if(!app.producteur){ setProducteur({ lat:latitude, lon:longitude }, { doReverse:true }); }
...
function onProjectChanged(){
  const pts=[]; if(app.producteur) pts.push(app.producteur); app.participants.forEach(p=>pts.push(p));
  // KPI + visuels
  updateComplianceKPI(); drawFeasibleZone(); drawMEC(); saveProject();
  // Sync chips mobile
  document.querySelectorAll('.mt-chipset .chip').forEach(b=> b.classList.toggle('active', Number(b.getAttribute('data-d'))===app.distMaxKm));
}

// ================== Bootstrap ==================
(function init(){
  // Ajuste le viewport (sécurité en plus de 100dvh/svh en CSS)
  (function tuneViewport(){
    const apply = ()=> document.documentElement.style.setProperty('--vh', (window.visualViewport?.height || window.innerHeight) + 'px');
    apply(); window.visualViewport?.addEventListener('resize', apply); window.addEventListener('orientationchange', ()=> setTimeout(apply, 150));
  })();
